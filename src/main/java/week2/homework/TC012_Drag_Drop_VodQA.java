...
public class TC012_Drag_Drop_VodQA {
...
//		Need to check for id which is not working
		common.click("xpath", "//android.widget.TextView[@content-desc='dragAndDrop']");

		WebElement dragele = common.waitForElement("accessibility id", "dragMe");
		
		WebElement dropele = common.waitForElement("accessibility id", "dropzone");
		
		new TouchAction<>(driver).press(ElementOption.element(dragele))
		.moveTo(ElementOption.element(dropele))
		.release()
		.perform();
		
		String successmsg = common.fetch_text("accessibility id", "success");
